...
def diagnotic1(lines: list[str]):
    ...
    for line in lines:
        ...
        if summary:
            diagnostics.append(base_actions.LineEdit(message=", ".join(summary), edit=None, source='Wildebeest', start=Position(line=lines.index(line), character=0), end=Position(line=lines.index(line), character=len(line))))
    if diagnostics:
        return diagnostics
    return False
